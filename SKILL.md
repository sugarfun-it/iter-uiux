...
- **Image mode**: trigger has one or more attached images. Always one-shot — single advisor call → apply `suggested_fix` → done. No Phase 0, no app launch, no recapture, no CHANGELOG (in-chat summary only). For another pass, paste a new screenshot and re-trigger; that *is* the loop, paced by the user. Optional signals refine the run: a red box / arrow / circle annotation marks a region as user-confirmed broken; free-form intent text (e.g. `多餘`, `對齊怪`) biases the fix direction. See `references/image-mode.md`.
...
- **Image mode: skip this entire phase.**
...
### Phase 1 — Environment prep
- **Image mode: minimal prep only.** Verify `codex exec --version` and detect the image flag (cache to `.iter-uiux/.runtime-state.json`). Skip app-launch checks, screenshot toolchain checks, and CHANGELOG creation.
...
### Phase 2 — Unit scan
- **Image mode: skip this phase entirely** (no CHANGELOG, no Detected Units table — unit is implicit from the image; see `references/image-mode.md`).
...
1. Round 1 (advisor): capture default-state screenshot, gather related source, draft flow outline. Call `codex exec` with the advisor prompt. See `references/codex-prompt.md`. **Image mode**: skip capture — attach the user-supplied image(s) as given, apply every `suggested_fix` from the response, then exit Phase 3 immediately (image mode never reaches step 2). See `references/image-mode.md`.
...
### Phase 5 — Summary
- Append the Summary block to `CHANGELOG.md` (units reviewed / converged, total rounds, files modified, duration).
- **Image mode**: do not write CHANGELOG. Reply in-chat with a brief summary — files changed, fixes applied, any unaddressable issue codex flagged.
...
1. **Fully automated after Phase 0.** Phase 0 is the only point that takes user input (one open question, plus at most one clarifying follow-up). Phases 1–5 must not ask the user for input. (Image mode skips Phase 0 — it is fully automated end to end.)
2. **Fresh scan every run.** Always re-scan all units. Never carry previous-run context into codex. (Image mode has no scan.)
3. **Archive, don't accumulate.** Each standard run starts a fresh `CHANGELOG.md`; old changelog files are renamed with a timestamp suffix. Archives are not fed to codex. (Image mode does not touch `CHANGELOG.md` at all.)
4. **Codex decides convergence.** Iterate until codex returns `done: true`. Only the anti-loop guard (failure-recovery.md) overrides this. (Image mode is one-shot — no convergence check; one advisor call, apply fixes, exit.)
